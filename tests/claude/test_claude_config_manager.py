...
class TestClaudeConfigManager:
    ...
    @pytest.mark.asyncio
    async def test_mcp_full_workflow(self, config_manager, temp_project_dir):
        """测试 MCP 完整工作流"""
        # 创建服务器配置
        server = MCPServer(
        ...
        # 扫描服务器
        mcp_info = await config_manager.scan_mcp_servers()
    ...
    @pytest.mark.asyncio
    async def test_markdown_full_workflow(self, config_manager, temp_project_dir):
        """测试 Markdown 完整工作流"""
        # 保存新的 command
        result = await config_manager.save_markdown_content(
            "command",
            "test-command",
        ...
        # 加载 command
        loaded = await config_manager.load_markdown_content(
    ...
    @pytest.mark.asyncio
    async def test_markdown_memory_scan(
        self, config_manager, temp_project_dir, temp_user_home
    ):
        """测试扫描 memory 文件"""
        # 创建各个 memory 文件
    ...
    @pytest.mark.asyncio
    async def test_markdown_scan_agents(self, config_manager, temp_project_dir):
        """测试扫描 agents"""
        (temp_project_dir / ".claude" / "agents" / "test-agent.md").write_text(
            "# Test Agent", encoding="utf-8"
        )

        agents = await config_manager.scan_agents()
    ...
    @pytest.mark.asyncio
    async def test_markdown_scan_commands(self, config_manager, temp_project_dir):
        """测试扫描 commands"""
        (temp_project_dir / ".claude" / "commands" / "test.md").write_text(
            "# Test Command", encoding="utf-8"
        )

        commands = await config_manager.scan_commands()
    ...
    @pytest.mark.asyncio
    async def test_hooks_full_workflow(self, config_manager):
        """测试 Hooks 完整工作流"""
        # 扫描空 hooks
        hooks_info = await config_manager.scan_hooks_info()
    ...
    def test_plugin_read_readme(self, config_manager, temp_user_home):
        """测试读取插件 README"""
        # 创建 marketplace 配置
        known_marketplaces = (
            temp_user_home / ".claude" / "plugins" / "known_marketplaces.json"
        )
        marketplace_path = temp_user_home / ".claude" / "plugins" / "test-marketplace"
        test_marketplace_data = {
            "test-marketplace": {
                "source": {
                    "source": "github",
                    "repo": "test/test-marketplace",
                },
        ...
        with open(known_marketplaces, "w", encoding="utf-8") as f:
            json.dump(test_marketplace_data, f)

        # 创建 marketplace 目录结构
        claude_plugin_dir = marketplace_path / ".claude-plugin"
        claude_plugin_dir.mkdir(parents=True, exist_ok=True)

        # 创建 marketplace.json
        marketplace_json_data = {
            "apiVersion": "1.0",
            "plugins": [
                {
                    "name": "test-plugin",
                    ...
                    "version": "1.0.0",
                    "source": "test-plugin",  # 相对路径
                }
            ],
        }

        with open(claude_plugin_dir / "marketplace.json", "w", encoding="utf-8") as f:
            json.dump(marketplace_json_data, f)

        # 创建插件目录和 README 文件
        plugin_path = marketplace_path / "test-plugin"
        plugin_path.mkdir(parents=True, exist_ok=True)

        readme_content = "# Test Plugin\n\nThis is a test plugin README."
        (plugin_path / "README.md").write_text(readme_content, encoding="utf-8")

        # 读取 README
        content = config_manager.read_plugin_readme("test-marketplace", "test-plugin")

        assert content == readme_content

    def test_plugin_read_readme_not_found(self, config_manager, temp_user_home):
        """测试读取不存在的插件 README"""
        # 创建 marketplace 配置
        known_marketplaces = (
            temp_user_home / ".claude" / "plugins" / "known_marketplaces.json"
    ...
    def test_plugin_read_readme_marketplace_not_found(
        self, config_manager, temp_user_home
    ):
        """测试读取不存在的 marketplace 的 README"""
        # 读取不存在的 marketplace 的插件 README
        content = config_manager.read_plugin_readme(
            "nonexistent-marketplace", "test-plugin"
        )
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_project_setup(
        self, config_manager, temp_project_dir, temp_user_home
    ):
        """测试端到端项目设置流程"""
        # 1. 设置 MCP 服务器
        ...
        # 2. 创建命令
        await config_manager.save_markdown_content(
            "command",
            "deploy",
    ...
    @pytest.mark.asyncio
    async def test_error_handling_invalid_operations(self, config_manager):
        """测试错误处理"""
        # 尝试删除不存在的 MCP 服务器
        ...
        # 尝试加载不存在的 markdown
        result = await config_manager.load_markdown_content(
    ...
    @pytest.mark.asyncio
    async def test_concurrent_access_simulation(self, config_manager, temp_project_dir):
        """测试并发访问模拟（MD5 检查）"""
        # 创建文件
        await config_manager.save_markdown_content(
            "command",
        ...
        # 获取 MD5
        original = await config_manager.load_markdown_content(
